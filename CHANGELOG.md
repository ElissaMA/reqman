...
- 版本报告以提醒单模板输出《工卡改版提醒单》（全量/逐包同款）：改版与作废同 sheet——改版 = 工卡名称（旧→新日期）浅蓝底、作废 = 工卡名称浅红底，按专业分列（电子/发动机/机体，与提醒单一致，特检/支援/其他不输出），飞机/工作包信息栏留空；标题与下载文件名统一中文格式「工卡改版提醒单（标识）查询完成日期」（全量标识=全量，逐包标识=机号+描述），下载名与预览页同路由同文件
