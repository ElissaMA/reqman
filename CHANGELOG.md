...
- 作废工卡库（独立 JSON `data/cancelled_cards.json`，与主库分离降低读写体积）：全量/逐包工卡版本检查检测到 AMRO 清单查无的库内工卡时，自动整卡移入作废库并从主库删除（承接工具/航材/提醒等全部原字段 + 作废时间/来源/原工卡组快照，同号 upsert 幂等）——作废卡自动退出工卡清单/工卡组/匹配/版本检查，不参与任何补查；数据管理新增「🗂 作废工卡」子页（仿工卡列表页：筛选 + 行点击详情弹窗含作废信息，仅查看+彻底删除，无恢复）
### Changed
...
- 工卡版本查询提速：SMJC/EOJC 两清单均加 `fleet=A320`（AMRO_CARD_FLEET，env 可覆盖，SMJC 1319→745、EOJC 5579→4627）；逐包检查定检例行卡（CSCA 前缀）走 SMJC 全量拉（秒级）、其余（EO/NRC/LS）改用 `TD_JC_ALL_GET_ENTITY_BY_JCNO` 按卡号直查（~50 张 ≈ 2 分钟），不再全量拉 EOJC 深分页（此前约 8 分钟）；新增按卡号直查端点入只读白名单（8 端点）
