...
## [3.6.1] - Unreleased
### UI（全站重设计）
- 工卡版本检查表述全链路统一为「检查」：按钮（检查工卡版本/检查版本）、进度卡与完成提示（检查完成）、持久摘要标签（全量检查工卡版本/检查工作包工卡版本）、改版清单标题与下载名（检查日期）、404 提示；工作包拉取/库存/飞机同步仍为「查询」
- 版本检查摘要细化（全量/逐包共用）：改版/新增/作废按专业细分（发动机 x、机体 x、电子 x、特检 x、支援 x…，零值省略），新增「检查总数 X 张」与「未检查 X 张」及原因说明（DP 项目 / 其他工卡 / 未录入主库——包内四家族卡号未录入卡库）；进度卡、toast 与持久摘要同步
- 按钮语义化体系：颜色=功能（AMRO 查询/登录=绿 `.btn-amro`、数据修改/保存=深蓝实心、进入编辑/本地处理=蓝描边、危险=红描边、下载=白底描边 `.btn-download`、中性导航=灰描边），形状/圆角/尺寸全站统一；表单内「新增工具/航材」由绿描边归入蓝描边、行内「重新匹配」归入蓝描边，消除绿色语义冲突；全部按钮配对对比度 ≥4.79:1（WCAG AA）
- 下载按钮统一命名与外观：全部改为「下载+内容」（下载需求单/下载提醒单/下载工卡改版清单/下载改版清单（全量）/下载逐包改版清单/下载库存查询/下载登录配置包），新增 `.btn-download` 统一组件（同尺寸描边+下载图标+统一 hover），全站 7 处下载按钮共用
- 逐包改版清单下载名规范化：工作包记录被清理后回退「工作包」而非 UUID，机号/描述中的文件名非法字符（如 `/`）统一替换
- 统一各输出文件下载行为：新增 `downloadFile` 助手（fetch + Blob，保留服务端中文文件名，失败统一 toast 不跳错误页），`a[data-download]` 事件委托统一直链；下载需求单/提醒单/工卡改版清单三个按钮全部走统一助手（含确认弹窗后 Blob 下载），不再整页跳转
...
- 工作包页移除「工卡版本变动日志」清单（未达预期作用，JSON API `/packages/amro-version-logs` 保留）；工卡清单「编写日期」列可点击排序，默认按「新建/编辑日期」列倒序展示，排序时空值恒排最后
...
### Fixed（正确性）
- 提醒单 B2 单元格未正确输出工作包描述：AMRO 导入的包只要定检级别（CHKTP）非空就显示「定检级别：4C」而非工作包描述（REVTITLE）；改为 B2 输出「定检描述：{工作包描述}」，描述缺失时回退定检级别（Excel 导入两字段同源不受影响）
